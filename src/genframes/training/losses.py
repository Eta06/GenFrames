...
class InterpolationLoss(nn.Module):
    ...
    def forward(
        ...
        if batch is not None and "flow_t0" in auxiliary and "flow_t1" in auxiliary:
            ...
            flow = functional_flow_loss(auxiliary["flow_t0"], target_flow0) + functional_flow_loss(
                auxiliary["flow_t1"], target_flow1
            )
        total = (
...
def functional_flow_loss(prediction: Tensor, target: Tensor) -> Tensor:
